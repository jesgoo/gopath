...
func TestAdd(t *testing.T) {
	...
	t.Log(cache.Add("Add", "Add", time.Minute, AbsoluteExpiration))
	t.Log(cache.Add("Add", "Add", time.Minute, AbsoluteExpiration))
...
func TestBasic(t *testing.T) {
	...
	cache.Set("int", 11, time.Minute, AbsoluteExpiration)
	cache.SetP("string", "string")
	...
	cache.DeleteMany([]string{"string", "float"})
...
func TestExpire(t *testing.T) {
	...
	cache.Set("a", 1, 2*interval, AbsoluteExpiration)
...
func BenchmarkGetM(b *testing.B) {
	...
	cache := NewMCache()
	for i := 0; i < count; i++ {
		cache.SetP(strconv.Itoa(i)+key, i)
	}
	key = "1000key"
	b.StartTimer()
...
func BenchmarkCacheSet(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		cache.Set(key, key, 0, AbsoluteExpiration)
	}
}
