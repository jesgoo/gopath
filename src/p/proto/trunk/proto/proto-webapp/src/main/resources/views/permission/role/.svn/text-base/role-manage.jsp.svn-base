...
<html>
		...
		<script type="text/javascript">
			...
			$(function() {
				$('#queryTable').datagrid( {
					title : '操作',
					//数据源
					url : '${ctx}/permission/role/query',
					//数据栏
					columns : [ [ {
						field : 'id',title : 'ID',width : perColW
					}, {
						field : 'name',	title : '角色名称',	width : perColW
					} ] ],
					//工具栏
	 				toolbar : [ {
						...
						text : '权限',iconCls : 'icon-tip',	handler : function(){permissionManage()}
					} ],
			    	//加载完成后
	...
	<body>
		...
		<!--修改-->
		<%@ include file="role-update.jsp"%>
		<!--删除-->
		<%@ include file="role-delete.jsp"%>
		<!--权限-->
		<%@ include file="permission-manage.jsp"%>
